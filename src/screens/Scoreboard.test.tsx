...
describe('Scoreboard', () => {
  it('displays all player names and balances', () => {
    render(<Scoreboard game={makeGame()} onAddPlayer={vi.fn()} onNewRound={vi.fn()} onEditRound={vi.fn()} onEnd={vi.fn()} />)
  ...
  it('calls onEnd when End Game button is clicked', () => {
    const onEnd = vi.fn()
    render(<Scoreboard game={makeGame()} onAddPlayer={vi.fn()} onNewRound={vi.fn()} onEditRound={vi.fn()} onEnd={onEnd} />)
    fireEvent.click(screen.getByRole('button', { name: /afslut spil/i }))
    expect(onEnd).toHaveBeenCalled()
  })

  it('allows entering a new player name and calls onAddPlayer', () => {
    const onAddPlayer = vi.fn()
    render(<Scoreboard game={makeGame()} onAddPlayer={onAddPlayer} onNewRound={vi.fn()} onEditRound={vi.fn()} onEnd={vi.fn()} />
)
