...
describe('Setup screen', () => {
  it('renders ruleset selector and player name inputs', () => {
    ...
    expect(screen.getAllByPlaceholderText(/player/i).length).toBeGreaterThanOrEqual(2)
  })
  ...
  it('start button is disabled until at least 2 player names are entered', () => {
    ...
    fireEvent.change(screen.getAllByPlaceholderText(/player/i)[0], { target: { value: 'Alice' } })
    expect(screen.getByRole('button', { name: /start/i })).toBeDisabled()
    fireEvent.change(screen.getAllByPlaceholderText(/player/i)[1], { target: { value: 'Bob' } })
  ...
  it('allows selecting Frants ruleset', () => {
    const onStart = vi.fn()
    render(<Setup onStart={onStart} />)
    fireEvent.change(screen.getByLabelText(/ruleset/i), { target: { value: 'frants' } })
    const inputs = screen.getAllByPlaceholderText(/player/i)
    fireEvent.change(inputs[0], { target: { value: 'Alice' } })
    fireEvent.change(inputs[1], { target: { value: 'Bob' } })
    fireEvent.click(screen.getByRole('button', { name: /start/i }))
